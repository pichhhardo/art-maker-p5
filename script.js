//    NOTE: these lines that start with two slashes are called COMMENTS.
//    Comments are ignored by the computer; they're great for writing notes to yourself!

// Let's write some code together!
//    Instructions for section 1:
//    https://github.com/LearnTeachCode/beginner-javascript-workshop/blob/master/1-variable-challenges.md


// Create a new variable
let myNerdyJoke = "Debugging is like being the detective in a crime drama where you are also the murderer";

// Display it in the console
console.log(myNerdyJoke);


let liz = "hellloo, I love JavaScript and hope you do too!";
console.log(liz);

let stefon = "From NC and is a huge Tar Heels fan" 
console.log(stefon);



let ali = "from Istanbul" + 5;
console.log(ali);


let gabe = aloha
console.log(gabe)

pauline = = test
console.log(pauline);

let jason = "JavaScript"
jason = 2018
console.log(jason)


let kat = "Hi";
console.log(kat);

let andy = "Hello world!";
